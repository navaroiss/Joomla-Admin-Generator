<?php
	...
	*/
	...
	class EventController extends JController implements Controller_Interface
	{

		var $c = '';
		var $option = '';
		var $tbl = 'agenda_event';
		var $pri_key = 'id';
		var $var = 'event';
		...
		function saveRecord($_POST, $action = '', $task='')
		{
			...
			foreach($_POST[$this->var] as $k=>$v)
			{
				...
				jimport('joomla.filesystem.file');
						$file_ext_allow['image'] = $this->_ext['image'];
$_some=array();foreach($this->_ext as $k=>$v){ if ($k == 'document' or $k == 'compress') $_some = array_merge($_some, $v); } $file_ext_allow['document'] = $_some;
		...
		function showRecord()
		{
			...
			if ($keywords!='') $find_keyword = "`content` like '%$keywords%'";
			
			if( count($_POST)>=1 ){
if($_POST[$this->var]['catid']>=1){
        $value = $_POST[$this->var]['catid'];        $join_query['select']  = "a.*"; 
        $join_query['table']  = "#__agenda_category a";
        $join_query['condition']  = "#__agenda_event.catid=a.id and #__agenda_event.catid = $value";}}
